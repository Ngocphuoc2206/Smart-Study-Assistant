...
const normalizeInputs = (reminders?: ReminderInput[]): NormalizedReminder[] => {
    ...
    return reminders
      .map((r) => {
        if (typeof r === "number") return { offsetSec: r, channel: "In-app" as const };
...
export const deleteByTask = async (userId: string, taskId: string) => {
    return Reminder.deleteMany({ user: new Types.ObjectId(userId), task: new Types.ObjectId(taskId)});
}

export const deleteBySchedule = async (userId: string, scheduleId: string) => {
    return Reminder.deleteMany({ user: new Types.ObjectId(userId), schedule: new Types.ObjectId(scheduleId)});
